...
→ **本アプリの設計**: 既定オフ＋初回同意ダイアログ、端末内処理が使える環境では自動優先し画面に「🔒端末内/☁️サーバー」を表示。音声で扱えるのは部位・左右・数値と、**合図語「患者」＋ID形式(英字0〜3文字＋数字1〜6桁)に合致する患者IDのみ**。日本語氏名はこの形式に合致しないため、発話しても患者IDとして採用されない(構造的な歯止め。単体テスト・ブラウザテストの両方で固定)。
